// ===============================
// Lazy-load carousel videos (only fetch/play when visible on screen)
// ===============================
function initLazyVideos(root = document) {
  const videos = root.querySelectorAll('.carousel-video');
  ...
  if (!('IntersectionObserver' in window)) {
    // Fallback: browsers without IntersectionObserver just load everything
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      ...
      if (entry.isIntersecting) {
        loadVideo(video);
      } else if (video.dataset.lazyLoaded === "1") {
        video.pause();
      }
    });
  }, { rootMargin: '200px' });
